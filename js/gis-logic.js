...
function generateHouseFootprint(lat, lng, seed, scale) {
    scale = scale || 0.0002;
    ...
    const w = scale * (0.6 + rand(1) * 0.8);
    const h = scale * (0.6 + rand(2) * 0.8);
    const angle = rand(3) * Math.PI;
    const corners = [
        [-w/2, -h/2], [w/2, -h/2], [w/2, h/2], [-w/2, h/2]
    ];
    const jitter = (n, mag) => (rand(n) - 0.5) * mag;
    const rotated = corners.map((pt, i) => {
        const x = pt[0] + jitter(i * 10 + 1, scale * 0.12);
        const y = pt[1] + jitter(i * 10 + 2, scale * 0.12);
...
function generateHouseFootprintByRisk(lat, lng, risk, seed, scale) {
    ...
    const rand = n => seededRandom(seed + n);
    const riskFactor = risk === 'danger' ? 1.6 : risk === 'major' ? 1.3 : risk === 'warning' ? 1.0 : 0.8;
    const w = scale * (0.8 + rand(1) * 1.0) * riskFactor;
    const h = scale * (0.8 + rand(2) * 1.0) * riskFactor;
    const angle = rand(3) * Math.PI;
    const corners = [
        [-w/2, -h/2], [w/2, -h/2], [w/2, h/2], [-w/2, h/2]
    ];
...
function renderHouseFootprint(item, idx) {
    const color = getHouseColor(item);
    const seed = item.no.charCodeAt(item.no.length - 1) + idx;
    const footprint = generateHouseFootprintByRisk(item.lat, item.lng, item.risk, seed, 0.00025);
    const polygon = L.polygon(footprint, {
        ...
        fillOpacity: 0.35,
        opacity: 0.8,
        className: 'house-footprint'
    }).addTo(map);
    polygon.bindPopup(buildHousePopup(item, idx));
    polygon.on('click', function() { selectHouse(idx); });
    footprintPolygons.push(polygon);
}
...
function initMap() {
    ...
    const boundaryPolygon = L.polygon(fengxianBoundary, { color: '#1a73e8', weight: 2, fillColor: '#1a73e8', fillOpacity: 0.08, dashArray: '5, 5' }).addTo(map).bindPopup('上海奉贤区范围');
    map.fitBounds(boundaryPolygon.getBounds().pad(0.05));
    renderMap();
}
...
function renderCommunityClusters(data) {
    ...
    renderCommunityFootprints(data);
}
function renderBuildingMarkers(data) {
    data.forEach((item, idx) => {
        renderHouseFootprint(item, idx);
...
function renderStreetFootprints(data) {
    data.forEach((item, idx) => renderHouseFootprint(item, idx));
}

function renderCommunityFootprints(data) {
    data.forEach((item, idx) => renderHouseFootprint(item, idx));
}
...
function switchTab(tab) {
    ...
    if (tab === 'basic') {
        ...
        body.innerHTML = '<div class="arch-section"><div class="arch-row"><div class="arch-label">销号状态</div><div class="arch-value"><span class="risk-tag ' + cfg.class + '">' + cfg.label + '</span></div></div><div class="arch-row"><div class="arch-label">申请时间</div><div class="arch-value">' + (info.applyTime || '未申请') + '</div></div><div class="arch-row"><div class="arch-label">审核时间</div><div class="arch-value">' + (info.reviewTime || '待审核') + '</div></div><div class="arch-row"><div class="arch-label">审核人</div><div class="arch-value">' + (info.reviewer || '-') + '</div></div><div class="arch-row"><div class="arch-label">备注</div><div class="arch-value">' + (info.note || '-') + '</div></div><div class="arch-row"><div class="arch-label">证明文件</div><div class="arch-value">' + (info.certFiles && info.certFiles.length ? info.certFiles.map(f => '<span class="file-tag"><i class="fas fa-file-alt"></i> ' + f + '</span>').join(' ') : '暂无') + '</div></div></div>';
    }
}
