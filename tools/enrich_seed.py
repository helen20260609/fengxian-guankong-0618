...
new_func = '''function generateHouseSeed() {
    ...
    // 真实的农村自建房风险分布：完好房/第二类占多数，危房占少数
    const RISK_DISTRIBUTION = [
        { risk: 'danger',  governanceWeights: { done: 0.35, doing: 0.30, overdue: 0.20, pending: 0.15 }, ratio: 0.10 }, // 第三类
        { risk: 'major',   governanceWeights: { done: 0.30, doing: 0.40, overdue: 0.15, pending: 0.15 }, ratio: 0.15 }, // 第三类
...
# 输出替换后的数据分布预览
print('风险分布权重：第三类25% | 第二类25% | 第一类35%')
